...
- [x] Verify it counts as the Render health check (see `render.yaml`)
- [ ] User: point UptimeRobot HTTP GET monitor at `https://api.ruralnexus.org/api/keep-alive` every 5 min
...
- [x] Registered in `payload.config.ts`
- [ ] User: verify `ruralnexus.org` domain in Resend (SPF/DKIM/MX in Hostinger DNS) — currently using `onboarding@resend.dev` as a stopgap
- [x] User: fill `.env` with `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `ADMIN_EMAIL` (dev: sender=`onboarding@resend.dev`, admin=personal gmail)
...
- [x] Confirmed row appears in `/admin/collections/contact-inquiries` and email lands at `ADMIN_EMAIL` (delivered to gmail, not in spam)
- [ ] Post-Resend-verification: flip `.env` to `RESEND_FROM_EMAIL=noreply@ruralnexus.org`, `ADMIN_EMAIL=info@ruralnexus.org` (both local and Render)
